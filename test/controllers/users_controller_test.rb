...
class UsersControllerTest < ActionDispatch::IntegrationTest
  test "redirects to feed after successful sign up" do
    ...
    follow_redirect!
    assert_select(".notification.is-success", text: I18n.t("users.create.welcome", name: "Jane"))
  end
  ...
  test "renders error when password confirmation doesn't match" do
    get(sign_up_path)
    assert_response(:ok)
    ...
    assert_no_difference(["User.count", "Organization.count"]) do
      post(sign_up_path, params: {
        user: {
          ...
          password: "password",
          password_confirmation: "asswordp"
        }
      })
    end
